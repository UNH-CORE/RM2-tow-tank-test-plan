...
parts---one for the processed data, and one for the raw data. This will allow
users to first obtain the software, processed data, and documentation---most
relevant for generating figures or comparing to numerical modeling
results---without having to download an excessively large archive. However, if
users want to reanalyze the raw data, the processing code will be written such
that raw data files are downloaded as needed, rather than one bulk download,
though it will still be possible to download the entire raw dataset, if desired.
