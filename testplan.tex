...
\begin{itemize}
	...
	\begin{itemize}
		\item Document the dependence of the turbine power coefficient on Reynolds number, and where it becomes independednt of Reynolds number.
...
test matrix ``sections,'' in which typically a single independent variable is varied. For
example, a test matrix section will be dedicated to collecting turbine performance
...
matrix sections will be created/executed for different tow speeds. For wake
characterization, test matrix sections will vary the cross-stream position of the velocity
probe, providing a single profile transverse, and multiple profiles
will be taken at varying vertical locations. Examples of test matrices for a performance 
curve and a wake profile are shown in Table~\ref{tab-test_section}.

Parameters for each test matrix section will be created in the \texttt{Config/Test
...
at a relatively slow tow speed (on the order of 0.2 m/s), incrementing up to the maximum
practical tow speed. For the scale of the RM2 model, we expect to see convergence in the measured power
...
An expanded uncertainty interval with 95\% confidence can be computed for $\lambda$,
$C_P$, and $C_D$
\begin{equation}
U_{95} = t_{95} u_c,
\end{equation}
...
matrices for each section of the test plan, whose names are inferred from the CSV file
names. Raw data files are stored in the \texttt{Data/Raw/\{section\}/\{run\}}
subfolders, while processed data are stored in the
\texttt{Data/Processed} subfolder---one CSV file per test matrix section.
...
\begin{itemize}
	...
	\item Test plan section and run number, e.g., ``\texttt{Perf-0.8 run 4}''
...
This experiment will likely produce on the order of 10 GB of data. The two guiding principles
...
The research products described above---documentation, data, code, CAD models, etc.---will be
