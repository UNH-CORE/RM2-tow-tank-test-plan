...
test ``segments,'' in which typically a single independent variable is varied. For
example, a test segment will be dedicated to collecting turbine performance
...
segments will be created/executed for different tow speeds. For wake
characterization, segments will vary the cross-stream position of the velocity
...
a test segment. Examples of test matrices for performances curve and wake
profile are shown in Table~\ref{tab-test_section}.

Parameters for each test segment will be created in the \texttt{Config/Test
...
matrices for each test segment. Segment names are inferred from the CSV file
names. Raw data files are stored in the \texttt{Data/Raw/\{section\}/\{run\}}
subfolders, while processed data from each segment are stored in the
\texttt{Data/Processed} subfolder---one CSV file per experiment segment.
...
\begin{itemize}
	...
	\item Run segment and number, e.g., ``\texttt{Perf-0.8 run 4}''
