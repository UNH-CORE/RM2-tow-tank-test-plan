...
Model 2 (RM2), to to provide
an open-source point design for MHK research and development, including its use as a test object for  scaled model testing to generate power performance data that can be used to validate open-source design tools.  More details on the reference
modeling effort are provided in Neary et al. \cite{Neary2014}.
...
number, Froude number and tip-speed-ratio of the model and the full scale device match.  Where
\begin{equation}
Re_L = \frac{UL}{\nu},
\label{eq-Re}
\end{equation}
...
respectively. For hydrokinetic turbine testing, $Re$, not $Fr$, isthe dominant scaling parameter.  However, theFroude number (based on blade tip submergence) should remain small (significantly
below one)It is rare to
...
horizontally). Thesevariation become larger as the tip speed
ratio decreases \cite{Para2002}. Typically, this variation is sufficiently large
...
investigate the accuracy of CACTUS and other numerical models. This project will provide insight on the physics of hydrokinetic
cross-flow turbines, including the importance of blade strut drag on turbine
power output, which will inform investigations on the present limitations of the CACTUS model and potential modifications to improve its accuracy.  The strut drag investigation will also give turbine designers some
perspective on the need to mitigate parasitic strut drag effects to improve rotor efficiency. The
experimental dataset will be shared publicly and can be used for validating
other numerical models/codes used by developers and DOE partners. The study
goals can be distilled into the following main objectives:
